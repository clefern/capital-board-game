// ========================================
// RandomEventManager - Eventos Aleatórios (Shared)
// ========================================
...
const EVENTS = [
  ...
  },
  {
    ...
    apply(gs, rng = Math.random) {
      for (const p of gs.activePlayers) {
        if (p.businesses.length > 0) {
          const biz = p.businesses[Math.floor(rng() * p.businesses.length)];
...
export class RandomEventManager {
  ...
  static tryTrigger(gameState, rng = Math.random) {
    if (rng() > this.EVENT_CHANCE) return null;
    const event = EVENTS[Math.floor(rng() * EVENTS.length)];
    event.apply(gameState, rng);
    return event;
  }
}
